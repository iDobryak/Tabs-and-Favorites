...
export function renderSiteGroups({
  ...
  if (siteGroups.length === 0) {
    elements.sites.textContent = t('noSitesForCurrentFilters');
    return;
  }
  ...
  for (const siteGroup of siteGroups) {
    ...
    meta.textContent = t('siteMeta', {
      tabsCount: siteGroup.totalTabs,
      urlGroupsCount: siteGroup.urlGroups.length,
      duplicateUrlGroupsCount,
      removableDuplicatesCount: duplicateIdsForSite.length
    });
    ...
    closeSiteDuplicatesButton.textContent = t('closeSiteDuplicatesButton');
    ...
    closeSiteButton.textContent = t('closeSiteButton');
    ...
    for (const urlGroup of siteGroup.urlGroups) {
      ...
      tabsSorted.forEach((tab) => {
        ...
        tabTitle.textContent = tab.title || t('untitledTab');

        const badges = document.createElement('div');
        badges.className = 'badges';
        badges.appendChild(
          createBadge(t('badgeWindow', { windowId: tab.windowId }))
        );

        if (duplicateTabIds.has(tab.id)) {
          badges.appendChild(createBadge(t('badgeDuplicate'), 'duplicate'));
        }

        if (isTabInGroup(tab)) {
          badges.appendChild(createBadge(t('badgeGrouped'), 'grouped'));
        }

        if (tab.pinned) {
          badges.appendChild(createBadge(t('badgePinned'), 'pinned'));
        }
